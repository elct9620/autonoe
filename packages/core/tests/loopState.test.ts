  ...
  describe('toResult', () => {
    it('LS-070: builds success result when AllPassed', () => {
      const state = LoopState.create()
        .incrementIterations()
        .addCost(0.5)
        .updateDeliverableCounts(3, 3, 0)
        .setExitReason(ExitReason.AllPassed)

      const result = state.toResult(1000)
    ...
    it('LS-071: builds interrupted result', () => {
      const state = LoopState.create().setExitReason(ExitReason.Interrupted)

      const result = state.toResult(500)
    ...
    it('LS-072: builds quota exceeded result', () => {
      const state = LoopState.create().setExitReason(ExitReason.QuotaExceeded)
    ...
    it('LS-073: builds error result with error message', () => {
      const state = LoopState.create()
        .recordError(new Error('Connection failed'))
        .setExitReason(ExitReason.MaxRetriesExceeded)
    ...
    it('LS-074: includes blocked count', () => {
      const state = LoopState.create()
        .updateDeliverableCounts(2, 5, 3)
        .setExitReason(ExitReason.AllBlocked)
